...
4. **Synthetic control.** For each of the six study neighborhoods (six high-violence coverage areas carried over from the original analysis, used as illustrative panels because synthetic control needs a readable number of them, rather than as the basis of the identification argument), weights over the 26 never-treated community areas are chosen by SLSQP to minimize pre-treatment squared error on annual gun-homicide counts, subject to the simplex constraint (weights non-negative, sum to one).
...
### 5.2 Signature one: the headline coefficient is an additive-scale artifact
...
### 5.3 Signature two: the pre-period cannot support parallel trends
...
### 5.5 Signature three: placement is near-deterministic, so there is no counterfactual
...
The consequence for identification is the selection-side statement of §5.7. A propensity model (violence, hardship, income) separates the two groups with an AUC of 0.94; the median propensity is 0.97 for ShotSpotter areas against 0.19 for never-treated areas; and the overlap failure is visible without picking any neighborhood by hand: the highest propensity among all 26 never-treated areas is 0.873, 36 of the 51 covered areas sit above that ceiling, and at a threshold of 0.90 the count is 35 treated against 0 of 26 never-treated ([`plots/placement_propensity_overlap.png`](plots/placement_propensity_overlap.png)). Those 36 areas span 0.6 to 37.4 pre-period gun homicides per year, so the failure is not confined to the violence tail; hardship drives it too. The six study neighborhoods carried over from the original analysis sit inside that band, each with a propensity of at least 0.97, but they illustrate the problem rather than establish it. The city installed ShotSpotter in essentially every highest-violence, highest-hardship neighborhood, leaving no comparable untreated unit from which to build a counterfactual, which is exactly why the synthetic control in §5.7 is not constructible.
...
### 5.6 Matching restores parallel trends, at the cost of the units in question

The natural response to a failing pre-trend test is to match treated and control units on pre-treatment observables before estimating. Because the propensity model above is exactly the object such a procedure would use, we can run that response on its own terms (`scripts/matched_did.py`, [`docs/results_summary_matching.md`](docs/results_summary_matching.md)).

Matching works, and the price is the finding. Restricting to comparable units does restore parallel trends: a joint Wald test on pre-period treated-by-year interactions rejects on the full panel (F = 2.67, p = 0.016) but no longer rejects on common support (F = 0.66, p = 0.70) or after 1:1 nearest-neighbour matching (F = 0.87, p = 0.55). The F statistic falls below one, so this is not only the loss of power that comes with a smaller sample, though with fewer than half the original units some of it is.

What the procedure costs is most of the treated distribution. Placement is predicted well enough that 36 of the 51 covered areas sit above every never-treated unit on the propensity, so only 17 of 51 find any control inside the caliper, and **none** of the six illustrative study neighborhoods survives either restriction. The design that satisfies parallel trends is a design about the marginal, lower-violence edge of coverage, not about the neighborhoods the program was built around. On that subsample the effect remains statistically null and less precisely estimated than before (IRR 1.31, 95% CI [0.97, 1.76] on common support; 1.20 [0.84, 1.72] matched).

This states the identification problem as a trade rather than a failure. These data can support a credible comparison, or they can speak to the neighborhoods the policy is about, but not both at once, and no reweighting of the units that exist can manufacture a counterfactual for units that have none.

### 5.7 Synthetic control is therefore not constructible
...
### 5.8 Replication with the treatment reversed: the September 2024 removal
...
**The common trend.** Gun homicides had been falling city-wide from their 2021–2022 peak for two full years before the shutoff, in ShotSpotter and never-treated areas alike. On a common indexed scale the two groups decline in near-lockstep straight through the removal date: coverage-area homicides fell 32% from the pre- to the post-removal window, but never-treated areas fell 23% over the very same months. The naive "crime fell after ShotSpotter left" comparison attributes a city-wide trend to the shutoff: the mirror image of a naive "crime rose after ShotSpotter arrived" comparison.

**The scale artifact.** Netting out the common trend with a difference-in-differences does not rescue identification, because the additive DiD reproduces the artifact of §5.2: it returns a "significant" removal *benefit* of −0.23 gun homicides per area-month (p = 0.001), but only because coverage areas start from a far higher baseline (0.85 versus 0.20 per area-month) and so move more in absolute terms for the same proportional change. On the correct multiplicative scale the removal effect is null (Poisson IRR 0.88, 95% CI [0.53, 1.46]). The post window is also short: six complete months, since administrative victim records are added retroactively and the two months nearest the data export are discarded as provisional, so the estimate is under-powered on top of being non-identified. Both the raw drop and the additive DiD are mirages: these data can no more identify the removal effect than the installation effect, in either direction, and the public debate has confidently drawn both unwarranted inferences.
...
Nothing in the diagnosis depends on gunshot detection. It depends on how the program was allocated, and allocation of that form is the norm rather than the exception in place-based policy.

The clearest precedent is the federal COPS hiring program, whose evaluation history traces the same arc. Evans and Owens (2007, *J. Public Economics* 91(1–2):181–201) estimated the effect of COPS grants on crime with a two-way panel design on a program awarded, like ShotSpotter, to the jurisdictions with the worst underlying problem. Worrall and Kovandzic (2007, *Criminology* 45(1):159–190) could not reproduce the result on comparable data, and the dispute persisted as long as it was fought over specification. What eventually moved it was not a better panel estimator but a different source of variation: Mello (2019, *J. Public Economics* 172:174–200) used the 2009 stimulus allocation formula to recover assignment variation plausibly unrelated to the outcome. When treatment is awarded *because of* the outcome, the binding constraint is the assignment mechanism, and progress comes from finding variation the mechanism did not determine, not from refining the estimator applied to variation it did.

Saturation targeting sharpens that lesson into a checkable condition. Limited overlap is a recognized hazard in the abstract (King & Zeng 2006; Crump et al. 2009), and the standard remedy is to trim to common support. Our matched analysis (§5.6) shows why that remedy can be empty in practice: trimming does deliver a sample on which parallel trends holds, but under saturation targeting the trimmed sample excludes every unit the program was built around, so the estimand it supports is not the estimand the policy question asks about.

Two implications follow for the many place-based interventions now allocated with administrative risk scores, from hot-spots policing (Sherman & Weisburd 1995; Braga et al. 2019) to targeted municipal services (Blattman et al. 2021). **First, the assignment model belongs at the front of the analysis, not in a robustness appendix:** an AUC near one for the units of interest is a stopping condition, not a footnote. **Second, a heterogeneity-robust staggered-adoption estimator is not a remedy here.** Those estimators repair weighting when treatment timing varies; they assume the overlap that saturation targeting destroys. The Goodman–Bacon decomposition makes the distinction operational: with only 4.6% of the weight on forbidden already-treated comparisons, the instability in this panel cannot be blamed on staggered timing, which is precisely how a researcher can tell a weighting problem from a support problem before concluding that a modern estimator has fixed anything.
...
The general lesson is the one we opened with. Saturation targeting is not a Chicago peculiarity or a ShotSpotter peculiarity; it is what competent, need-based allocation looks like from an evaluator's point of view, and it is becoming more common as cities target programs with administrative data. Where it holds, the binding constraint is the assignment mechanism rather than the estimator, and no amount of methodological sophistication applied after the fact will recover a counterfactual the rollout never generated. The practical implication is a change in order of operations: estimate the assignment model first, check overlap for the units the decision is actually about, and treat that check as a precondition for estimating an effect rather than as a robustness appendix.

A non-identification reached that way is not a failed evaluation. It is an accurate account of what the available data can support, which is what a procurement decision of this kind deserves.
