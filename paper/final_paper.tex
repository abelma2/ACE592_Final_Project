...
and improve real-time awareness of gunfire (Choi, Librett, \& Collins, 2014).
...
The evaluation record on gunshot detection is, by now, fairly settled. Studies at the
police-district level in Chicago (Connealy et al., 2024) and across metropolitan counties
nationally (Doucette et al., 2021) find no reduction in shootings, homicides, or case
clearance---only faster detection and more recovered evidence---and a Chicago district-level
study finds detection even slowed dispatch and lowered arrest probability (Topper \&
Ferrazares, 2024). The one channel with a credible claim to benefit is medical: a boundary
regression discontinuity attributes a lower shooting-fatality rate inside coverage to faster
emergency response (University of Chicago Crime Lab, 2024, not peer-reviewed). Against that
...
mid-2017 and mid-2018, leaving 26 never-treated areas. The original project pipeline
...
PM to 1 AM); Figure~\ref{fig:hourly} shows the within-day co-movement. As a
\emph{measurement instrument}, ShotSpotter works. The open question is whether detection
translated into fewer homicides.
...
bottoming out mid-morning and peaking late at night. The system detects the gunfire it
...
+7.51$ under OLS ($p = 0.009$), but should be re-estimated under Poisson for the same
reason; that exercise is left to follow-up work and is unlikely to overturn the
qualitative pattern.
...
COVID), with the pooled DiD estimate at the bottom; significant changes in red,
non-significant in green. The spread across neighborhoods---from roughly $-2\%$ to
...
the parameter is not identified in these data, rather than restoring it.
...
socioeconomic indicators. Two facts emerge. First, placement tracks structural
...
per-capita income with $0.92$, against $0.79$ for the pre-period gun-homicide count, and
...
(Figure~\ref{fig:overlap}). This is the selection-side statement of the counterfactual
problem: the city installed ShotSpotter in essentially every highest-violence,
...
tracked Chicago's racial geography at least as tightly as it tracked violence or
hardship---the concentrated-surveillance pattern advocacy groups described. (Community-area
race is measured from the 2019--2023 ACS, which post-dates the rollout; because Chicago's
neighborhood racial composition is highly persistent, we read it as a proxy for the
pre-rollout map.)
...
peer-reviewed). A mortality reduction of that magnitude falls squarely \emph{inside} our
own count-model confidence interval, and a community-area-by-year homicide panel---which
...
% ===========================================================================
\section{Why Detection Did Not Reduce Violence}
...
naming explicitly. The panel is annual, masking within-year dynamics. Standard errors
cluster on community area (about 77 clusters); a wild-cluster bootstrap would be more
conservative with this few clusters. The non-fatal-shootings outcome is still estimated
under OLS and should be re-estimated as a count model. Two event-study implementations
...
Topper, M., \& Ferrazares, T. (2024). \emph{The unintended consequences of policing
technology: Evidence from ShotSpotter}. Working paper (SSRN 4828813).
...
evaluations. \emph{Journal of Quantitative Criminology}.
