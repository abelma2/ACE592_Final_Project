...
homicides in either direction. The same non-identification recurs on an independent
event---Chicago's September 2024 shutoff, where gun homicides were already falling
city-wide in coverage and comparison areas alike, so the widely cited post-removal decline
reflects a city-wide trend, not a removal effect. They rule out a large \emph{deterrence}
effect, but cannot
...
the homicide series: the additive OLS coefficient is a scale artifact. The same artifact
...
shutoff, the mirror image of the naive ``crime rose after ShotSpotter arrived'' comparison
...
removal effect is null (Poisson IRR $0.91$, 95\% CI $[0.55, 1.52]$). The post window is also
