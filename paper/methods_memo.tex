...
% Austin Belman. Single-author, single-spaced working paper.
...
$+2.63$ homicides per community-area-year ($p < 0.001$). I show that the estimate is an
...
gun homicides in either direction, and they are inconsistent with the large reduction a
\$3 million detection contract was expected to produce. The methodological lesson
...
The policy question is whether the program reduced gun violence. The methodological
question, which I treat as primary, is whether the available data can answer the policy
question at all. I estimate the effect of ShotSpotter installation on gun homicides using
a community-area-by-year panel for 2009 to 2023, and I hold each estimate against the
assumptions its design requires. The headline difference-in-differences estimate is
...
comparable levels. I test the conditions the design needs and report each test against the
...
One conclusion survives. The data are inconsistent with a large reduction. An intervention
large enough to justify a \$3 million detection contract would leave negative
post-treatment coefficients with intervals excluding zero, stability across
specifications, and a clearer signal in the higher-powered non-fatal-shooting series,
which carries about 30,000 events. None of these appears. Whatever effect ShotSpotter had
on homicides was too small to separate from noise, from the 2016 anomaly, and from
pre-existing trend differences across neighborhoods.
