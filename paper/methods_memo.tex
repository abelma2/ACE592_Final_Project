...
% Austin Belman. Single-spaced working-paper version of the full paper.
...
2009 to 2023); the historical ShotSpotter alert file (222,108 alerts, January 2017 to the
program's termination on September 22, 2024; 191,824 fall in the 2017--2023 analysis
window); the 2008 to 2012 Census socioeconomic indicators; and the community-area
...
each, and the result.}
\label{tab:threats}
\begin{tabular}{p{0.22\textwidth}p{0.24\textwidth}p{0.28\textwidth}p{0.13\textwidth}}
\toprule
...
from zero (Figure~\ref{fig:spec}). Because the plain Poisson is itself a two-way fixed-effects
PPML, which under the staggered rollout inherits the same heterogeneity-weighting concern as
linear TWFE, we confirm the null with a heterogeneity-robust Poisson extended two-way
fixed-effects estimator (Wooldridge, 2023): the overall ATT incidence-rate ratio is $1.05$
(95\% CI $[0.79, 1.33]$, community-area cluster bootstrap), with cohort-time effects at one
at every horizon. The null is not an artifact of the plain PPML's fixed-effects weighting.
...
construction fails. We fit it for six high-violence coverage areas carried over from the
original analysis (Austin, Humboldt Park, North Lawndale, Englewood, West Englewood, and
South Shore), a set chosen to keep the number of panels readable rather than to carry the
argument. For all six the optimizer places 100\% of the
...
treated unit, and the failure is not a property of those six panels. A logit of coverage
on pre-period gun homicides, the hardship index, and per-capita income separates the two
groups almost completely (AUC $0.94$): 36 of the 51 treated areas carry a higher predicted
probability of coverage than every one of the 26 never-treated areas, and no never-treated
area exceeds $0.87$. Those 36 span $0.6$ to $37.4$ gun homicides per year, so the overlap
failure is not confined to the violence tail. The infeasibility is a property of the data: the city installed ShotSpotter
...
All inputs are public City of Chicago Data Portal products (homicides/shootings
\texttt{gumc-mgzr}; ShotSpotter alerts \texttt{3h7q-7mdb}; Census indicators
\texttt{kn9c-c2s2}; ACS race \texttt{t68z-cikk}; community-area boundaries
\texttt{cauq-8yn6}). The full reproducible pipeline is at
